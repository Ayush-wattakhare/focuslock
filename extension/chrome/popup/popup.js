// FocusLock Browser Extension - Popup Script
...
function setupEventListeners() {
  ...
  document.getElementById('disconnect-btn')?.addEventListener('click', async () => {
    if (confirm('Are you sure you want to disconnect? Your lock rules will no longer sync.')) {
      await chrome.storage.local.remove(['apiToken', 'lockRules', 'lastSync']);
      showAuthSection();
    }
  });
}
...
async function loadCurrentSiteStatus() {
  try {
    ...
    // Check lock status
    const lockStatus = await chrome.runtime.sendMessage({
...
async function loadLockRules() {
  try {
    const { lockRules } = await chrome.storage.local.get('lockRules');
