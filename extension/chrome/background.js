...
async function evaluateLockStatus(domain) {
  try {
    const { lockRules, apiToken } = await chrome.storage.local.get(['lockRules', 'apiToken']);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'SET_API_TOKEN') {
    chrome.storage.local.set({ apiToken: message.payload.token }).then(() => {
      syncLockRules();
      sendResponse({ success: true });
    });
    return true;
  ...
  if (message.type === 'GET_LOCK_RULES') {
    chrome.storage.local.get(['lockRules', 'lastSync']).then(sendResponse);
    return true;
  }
});
